...
no <- c(1:11,12,14,16,18:23,25:30,32,34,36:38,41,42,43,45,47,51:53,55,57:59,61,62,64,65,67,69:76,78,81,82,86:89,91,92,94:97,99,101,104,107,109:116,
...
carac[carac$`Name of the clustering method used` == "k-means | Multinomial Mixture Model" & carac$`Type of clustering method` == "More than one type","Type of clustering method"] <- "Model-based Clustering"
carac[carac$`Name of the clustering method used` == "k-means" & carac$`Type of clustering method` == "More than one type","Type of clustering method"] <- "Partitional Clustering"
carac[carac$`Name of the clustering method used` == "PCA" & carac$`Type of clustering method` == "High-dimensional Clustering","Type of clustering method"] <- "Others"

tipusxmet2 <- table(carac$`Type of clustering method`,carac$`Name of the clustering method used`)
(tipusxmet2 <- prop.table(tipusxmet2))
...
ggplot(carac, aes(x = reorder(`Type of clustering method`, `Type of clustering method`, function(x)-length(x)), fill = `Name of the clustering method used`)) +
  ...
  scale_y_continuous(labels=percent) + scale_fill_brewer(palette = "Set3") + labs(x='Type of clustering method',y='Percentage',fill='Name of the clustering method used') + theme_bw() 
  + theme(axis.text = element_text(size=9.5), axis.title = element_text(size=18), legend.text = element_text(size = 12))
...
freq <- c(nrow(carac[carac$`Publication Year` == 2007,]),nrow(carac[carac$`Publication Year` == 2008,]),
...
data <- data.frame(Year = 2007:2022,
                   n=freq)
windows()
ggplot(data, aes(x=Year, y=n)) +
  geom_line(stat="identity", size = 1.5) + ylim(0,12.5) + theme (plot.title = element_text(vjust=2, face="bold", 
  color="black", lineheight=1.5))+ labs(x='Year', y='Number of articles') + geom_smooth(se = F, size = 1.5) + theme_bw() + theme(axis.text = element_text(size=18), axis.title = element_text(size=18))
...
# It deletes the observation 220 and repeat everything
